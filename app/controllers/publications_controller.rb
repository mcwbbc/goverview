class PublicationsController < ApplicationController
  ...
  # GET - displays all pubs
   def index
      top_pubs_sql = "select publications.id, publications.title, count(annotations.id) as total from publications, annotations where annotations.publication_id = publications.id group by publications.id order by total desc limit 20"
